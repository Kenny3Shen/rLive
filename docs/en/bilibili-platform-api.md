...
| One normal chat message | Supported | Requires the local send switch and a valid logged-in session. |
...
`danmaku_connect` opens the receiving connection and refreshes its short-lived data after a reconnect. `bilibili_danmaku_send_status` and `bilibili_danmaku_send` are deliberately narrow internal commands for one user-initiated ordinary message. Super Chat is display-only; rLive has no payment or gift write flow.
...
rLive sends only one ordinary text message per explicit user action. It has no bulk, loop, schedule, auto-reply, gift, payment, or automatic retry path. A resolved write command never synthesises chat locally: the list and floating layer render the message only after the normal room connection receives the platform's real echo.
