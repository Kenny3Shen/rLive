...
`PlayerPane` owns the Bilibili/Douyu/Huya room-session auto-send scheduler above `DanmakuSettingsPanel`, so it continues while the settings tab is hidden. It is frontend-only: it reuses the existing per-platform single-message command and validation/status checks, and adds no Tauri command, database field, `AppSettings` field, or profile data. Its switch and text are default-off session state. It can start only while the shared local permission, current platform Cookie/send status, and text validation are valid. On enable it waits 20 seconds, normalises line breaks and repeated whitespace to one space, then splits text by grapheme into fragments of at most 15 user-visible characters that also fit the platform UTF-16 limit; a single over-limit grapheme is a validation error. Sends are serial, start at least 20 seconds apart, and loop from the final fragment to the first. Editing text, switching rooms (which clears the session text), leaving/unmounting the player, closing the app, or any send failure stops the session immediately and exposes the reason; a failed or ambiguous write is never retried automatically.
...
Persisted settings are `danmaku_area`, `danmaku_line_count`, `danmaku_opacity`, `danmaku_font_size`, `danmaku_font_weight`, `danmaku_speed`, `danmaku_filter_repeats`, `danmaku_filter_gifts`, `danmaku_shield_words`, `danmaku_send_enabled`, and `iptv_custom_m3u_url`. `danmaku_send_enabled` defaults to `false` and, at the top of Account settings, jointly controls Bilibili, Douyu, and Huya sending. Auto-send text, fragment position, and enabled state are deliberately absent from persisted settings, the database, and profile packages. Cookies stay in the separate local `cookies` table. Profile export omits `danmaku_send_enabled` and `iptv_custom_m3u_url`; import preserves their local values so an untrusted profile cannot enable sending or replace a private playlist address.
