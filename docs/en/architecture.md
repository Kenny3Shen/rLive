...
- `DanmakuSettingsPanel` provides Simple Live-style room-side controls and receives the PlayerPane auto-send controller for the current session; `FollowPanel` ranks live follows first and replaces the room route, so its Back action returns home instead of a previous live room.
