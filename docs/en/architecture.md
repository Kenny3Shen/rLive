...
`DanmakuEvent` has optional `SuperChatInfo` and ordered `DanmakuContentSpan` fragments. The Bilibili `SUPER_CHAT_MESSAGE` parser validates and forwards its ID, price, currency, duration, and colour metadata. The frontend uses the validated primary and bottom Bilibili tier colours for a compact amount-header gradient, while a neutral card body keeps the full message readable. Normal Bilibili chat reads image-emote metadata from `info[0][13].url` and `info[0][15].extra.emots`; only HTTPS Bilibili CDN URLs reach the frontend and text/image order is retained.
...
- `superChat.ts` formats safe amounts/durations and turns only validated hexadecimal colours into the compact SC amount-header gradient, never passing raw websocket values to style props; `danmaku/filter` precompiles shield matchers and maintains bounded content aggregators.
