...
| Douyin | Anonymous first-page recommendations/categories; search needs login | Yes | Yes (configured signer required) |
...
Douyin's WSS endpoint requires a short-lived signed URL. rLive does not ship a reverse-engineered signer or bundle a signer service. Before entering a chat-enabled room, set the complete endpoint of a service you operate or trust under **Settings → Account → Douyin real-time danmaku**. HTTPS endpoints and HTTP endpoints on `localhost`, `127.0.0.1`, or `::1` are accepted. See [Douyin platform API documentation](douyin-platform-api.md).
...
Entering a Bilibili, Huya, Douyu, Douyin room with a configured signer, or Twitch room connects that site's danmaku service. Chat appears in the side list; optional floating tracks overlay the video.
...
| Douyin chat cannot reach the signer | Under Settings → Account → Douyin real-time danmaku, verify the complete configured endpoint is running and returns a valid WSS URL. Only HTTPS or loopback HTTP (`localhost`, `127.0.0.1`, `::1`) is accepted. |
