...
| Auto-send danmaku (Bilibili / Douyu / Huya) | Default-off, current-session-only text and switch; requires the shared send permission and current platform readiness, then sends validated 15-grapheme-or-less fragments every 20 seconds as described above |
