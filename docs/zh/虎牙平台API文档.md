...
| 能力 | 状态 | rLive 行为 |
| --- | --- | --- |
| 分类、推荐、分区房间、搜索 | 已支持 | 分类走两级目录接口，其余解析虎牙网页与公开配置数据，按上游分页结果展示。搜索同时覆盖在播房间与未开播主播。 |
...
搜索走 `https://search.cdn.huya.com/?m=Search&do=getSearchContent&q=&uid=0&v=1&typ=-5&start=&rows=`，响应是多路索引，其中两路有用：

| 索引 | 覆盖范围 | 关键字段 |
| --- | --- | --- |
| `response.3` | 只有在播房间 | `room_id`、`game_introduction`（直播标题）、`game_screenshot`、`game_total_count`（当前人数） |
| `response.1` | 全部主播，含未开播 | `room_id`、`game_nick`、`live_intro`、`gameLiveOn`、`game_avatarUrl180` |

分页约定（这里曾经有个静默失效的 bug）：

- `startPage` 是**空转参数**，任何取值都返回第一页。真正生效的偏移是 `start`，并且服务端按 `floor(start / rows)` 归桶，因此 `start` 必须是 `rows` 的整数倍：`start = rows * (page - 1)`。实测 `rows=40` 连翻七页得到 280 个互不重复的 `room_id`，零重叠；「英雄联盟」的 `numFound` 约 1.5k，可翻深度以它为准。
- 只有主播索引响应 `start`；在播索引 `response.3` 完全忽略偏移，第二页再读会把第一页的房间原样重复带出来，因此**只在第一页读在播索引**，后续页的在播房间由主播索引补上（`gameLiveOn` 为 `true`）。
- `has_more` 只看主播索引：主播索引返回满页且 `response.1.numFound` 还有剩余时才继续。
- 主播索引没有当前观看人数字段。`game_activityCount` 是累计活跃度，与在播索引的当前人数不同量纲，因此这些条目热度留 0 表示未知，不换算、不硬凑。
- 未开播主播的 `live_intro` 仍然有内容，是上一场或预告的标题（实测「解说一下今天的比赛！」），拿它当直播标题会让卡片看起来还在播，因此 `gameLiveOn` 为 `false` 时标题留空，由前端退回主播名。
