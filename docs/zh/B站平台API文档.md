...
| 能力 | 状态 | rLive 行为 |
| --- | --- | --- |
| 分类、推荐、分区房间、搜索 | 已支持 | 使用网页公开可访问的直播与搜索数据；列表可按上游返回继续加载。搜索同时覆盖在播房间与未开播主播。 |
...
接收普通弹幕通常可匿名工作；扫码或手动 Cookie 登录可改善网页登录态兼容性。图片表情只加载经过来源校验的 B 站 CDN 图片，整条消息的装扮大表情在飘屏中占两条车道并放大，内联小表情与文字同行；SC 仅展示平台下发的事件。
