...
- 页签条之上的 UP 主信息卡：头像与 UP 名点开投稿抽屉，名字右侧显示稿件发布时间（`formatRelativeTime`，与评论区的相对时间同一实现）；统计行（播放/弹幕/评论）沿用稿件详情数据。发布时间来自 `VideoArchive.pubdate`。列表页 UGC 卡片同样显示发布时间（`VideoItem.pubdate`）：推荐/热门/搜索自带 Unix 秒，UP 主投稿列表只给 `created`（北京时间字符串 `yyyy-MM-dd HH:mm`），后端 `created_to_unix` 按 UTC 解析后减 8 小时还原，缺失为 0 前端不渲染；卡片标题恒占两行（`min-h-[2lh]`），一行标题的卡片靠占位把 UP 主行与统计行压到相同纵向位置。
...
- 桌面普通详情保留流内 `topBar` 和右侧投屏 Popover。移动端直播与视频均取消流内顶栏，返回、标题和低频工具改在播放器顶部覆盖显示，与底部控制栏共用空闲显隐；画面占满原顶栏空间，HUD 自行避让状态栏/刘海。
- 移动端与全屏的低频工具统一由 `PlayerHudOverflowMenu` 承载：投屏 / 复制链接 / 在浏览器中打开；移动端普通视频还保留返回主页入口。投屏使用 `PlayerToolPanel` 与 `CastMenu`，进行中显示「投屏中」。
- 顶栏工具与 HUD 共用 `castOpen` 和 `castMenuProps`，互斥挂载。有效播放舞台在 `mobileClient || shortVideo || fullscreen || webFullscreen` 时接管顶栏；无效参数、PGC 解析态继续渲染带安全区的流内顶栏，保留返回入口。
...
- 移动端普通视频、短视频和两种全屏都使用舞台顶部 HUD。短视频 HUD 常驻，返回来源列表；普通视频沿用控制栏空闲显隐，返回先退全屏、无全屏层时返回页面（从短视频展开的详情优先返回短视频）。固定沉浸层/画面全屏的菜单与 toast 进入 `stageRef`，停用侧栏的 `DrawerViewport`，避免浮层出现在隐藏的详情区。
...
- 底部 Shell 仅桌面端显示链接操作，移动端统一从播放器 HUD 菜单访问。未挂载舞台的解析/错误顶栏仍保留 `VideoMobileActions`；详情侧栏保留底部安全区 padding。
