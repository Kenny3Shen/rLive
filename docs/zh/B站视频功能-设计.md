...
- **弹幕卡正文封顶（无限滚动的根因）**:窗口化列表的总高度是「已测行实测高 + 未测行估高」之和,而实测发生在行**进入视口时**。弹幕正文会换行、高度随字数变化,实测系统性大于估高(132)时,往下滚就不断把总高度往上抬:实测每滚 400px 内容增长最多 252px,214 步里 210 步在增长,总高 53.9k → 86.5k(+61%)——滚动条比滚动更快变长,表现就是「怎么也滚不到底」。修法是把正文封顶在约三行(`DANMAKU_CONTENT_MAX_HEIGHT_PX = 66`,用 `max-height` 而非 `line-clamp`,全文仍可读),并把估高提到封顶后的卡高(`DANMAKU_CARD_ESTIMATE_PX = 170`)。两者必须一起改:只封顶而估高仍为 132 时,封顶后实测 168 依旧高于估高,增长步数仍有 164/166;提到 170 后增长步数为 0。估高略高于实测只会让内容反向收缩,无害。
