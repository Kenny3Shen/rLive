...
更新时间：2026-07-28。本页说明 rLive 对抖音直播网页数据、播放和实时弹幕的接入边界；它不是抖音开放平台或直播 SDK 的官方开发文档。
...
| 弹幕发送 | 未支持 | 当前签名只用于接收连接，不能推导为写入权限。 |
...
抖音实时弹幕连接地址受短时签名保护。rLive 按与 Simple Live 相同的流程在本机完成：
