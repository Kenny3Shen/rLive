...
| 能力 | 状态 | rLive 行为 |
| --- | --- | --- |
| 分类、推荐、分区房间、搜索 | 已支持 | 使用网页/H5 读取接口；推荐、分区与搜索按上游分页结果展示。搜索同时覆盖在播房间与未开播主播。 |
...
| 账号 | 已支持 | 可扫码登录或手动保存完整 Cookie；只保存在当前设备。设置页读取账号摘要时探测登录态，失效会提示并自动退出。 |
...
搜索按关键词同时请求两路索引，两路共用同一个 `page`、每页 `pageSize=20`：

| 索引 | 地址 | 覆盖范围 | 关键字段 |
| --- | --- | --- | --- |
| 房间 | `japi/search/api/searchShow?kw=&page=&pageSize=` | 只有正在直播的房间 | `rid`、`roomName`、`nickName`、`roomSrc`、`hot` |
| 主播 | `japi/search/api/searchAnchor?kw=&page=&pageSize=` | 全部主播，含未开播 | `rid`、`nickName`、`avatar`、`isLive`、`isLoop`、`description` |

解析约定：

- 在播判定统一为 `isLive == 1 && isLoop != 1`。`isLoop == 1` 是视频轮播房间（实测 `rid 9804176`），与 `live_status_from_room_info` 里 `show_status == 1 && videoLoop != 1` 的口径一致，因此搜索角标和房间页不会互相矛盾。
- 房间索引的条目先入列（它带真实直播标题），主播索引给出的开播判定优先，因为只有它带 `isLoop`。两路按 `rid` 去重。
- 主播索引的 `description` 是主播签名而不是直播标题（`desType` 0/1 时尤为明显），因此这些条目的 `title` 留空，由前端退回主播名，不把签名当标题展示。
- `hot` 是本地化标签（如 `237.2万`）而非原始数字，走 `parse_online_label` 近似还原；主播索引没有这个字段，未开播条目热度为 0。
- 翻页只要任一索引还有下一页就继续。主播索引比房间索引深得多（实测「旭旭宝宝」909 条命中，前三页 `rid` 互不重复），房间索引先耗尽时不该把滚动停在那里。

搜索接口有独立风控：短时间连续深翻会返回 `{"error": 8, "msg": "您的行为可能存在风险, 请完成验证"}` 且 `data` 为空对象。适配器把 `error != 0` 当成错误而不是「没有结果」，并且只在两路索引都失败时才向上报错——一路被限流时先把另一路的结果给出来。
...
斗鱼没有第一方的账号资料读接口，因此 `account_get_profile` 用只需要登录态、返回体最小的关注列表首页作为会话探针：`wgapi/livenc/liveweb/follow/list?sort=0&cid1=0&offset=0&limit=1`。请求保留已保存的设备字段，仅在缺失时补上与搜索一致的兜底 `dy_did` / `acf_did`，并遵守应用代理设置。

判定对「失效」保持保守，因为设置页会据此自动退出登录：

| 应答 | 判定 |
| --- | --- |
| `error = 0` | 平台确认该会话，账号状态为有效。 |
| `error = -1`（`msg` 为「用户未登陆或token已过期」） | 平台明确拒绝，账号状态为已失效。未登录与已过期共用这条应答。 |
| 其它非零 `error`、非 JSON、缺字段、HTTP 失败或网络错误 | 一律留在「未知」，不改动已保存的 Cookie。 |

其它非零取值是业务或风控失败（例如搜索接口的 `error = 8` 需要完成验证），不能当作会话已失效。显示名来自 Cookie 自身的 `acf_username`，因此探针不可用时仍然可用。
