...
| 单条普通弹幕发送 | 已验证 | 个人直播间已成功发送；仍按平台确认和真实回显处理每一次发送结果。 |
...
实时能力由 `danmaku_connect` 和 `douyu_danmaku_send_status` / `douyu_danmaku_send` 提供。接收与发送使用不同网关职责：接收链路负责房间事件，发送链路负责当前账号的一条文字消息；两者都遵守应用代理设置。
...
发送前需要开启默认关闭的本机 `danmaku_send_enabled`，并保存完整的当前账号 Cookie。发送只接受数字房间号、非空单行文本和按房间 3 秒本机冷却；不提供批量、循环、定时、自动回复、礼物、支付或自动重试。
