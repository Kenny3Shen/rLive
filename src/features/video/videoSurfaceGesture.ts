...
/** 画面宽度的下限（px）：小窗不至于把每像素的时间放大到无法控制。 */
 ...
 * 手势可以接手（纵向已被左右半屏亮度/音量拿走、斜向拖动），
 * 此时只作废点按与长按，不提交任何动作。
 */
export type VideoSeekGestureIntent = "pending" | "seek" | "reject";

/**
 * 按位移判定这次按压是否交给横向 seek。
 *
 * `seekable` 是调用方在 pointerdown 时算出的候选资格（可 seek 的媒体），
 * 这里只做几何判定，不重复业务条件。
 */
export function videoSeekGestureIntent(
  deltaX: number,
  deltaY: number,
  seekable: boolean,
  ...
  if (
    ...
    return seekable ? "seek" : "reject";
  }
  return "reject";
}

/** 本次拖动能覆盖的 seek 跨度（秒）：以自身长度为上限，避免整屏就跨完全片。 */
