          ...
          <TooltipTrigger
            render={
              <Button
                ...
                disabled={!favoriteEnabled || favoriteBusy}
...
export function IptvPlayerPage() {
  ...
  const favoriteSourceId = favoriteSourceOverride ?? iptvFavoriteSourceId(source);
  const favoritesQuery = useIptvFavorites(favoriteSourceId, sourceIsValid);
  ...
  const playlistQuery = useQuery({
    ...
    enabled: channelUrl !== null && sourceIsValid && isHttpUrl(source.url),
    staleTime: 5 * 60_000,
    gcTime: 15 * 60_000,
  });
  const channel = useMemo(() => {
    return resolveIptvChannel(channelUrl, playlistQuery.data, favoritesQuery.data);
  }, [channelUrl, favoritesQuery.data, playlistQuery.data]);
  ...
  const topBar = (
    <IptvPlayerTopBar
      ...
      favoriteEnabled={channel !== null && !favoritesQuery.isLoading}
      onBack={goBack}
  ...
  if (
    ((playlistQuery.isPending && isHttpUrl(source.url)) || favoritesQuery.isPending) &&
    !channel
  ) {
    return (
      <div className="flex h-full min-h-0 flex-col bg-background">
