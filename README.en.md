...
rLive is a desktop live client built with **Tauri 2, React, TypeScript, and Rust**. It provides multi-site browsing, playback, danmaku, and device-local data management.
...
| Douyin real-time danmaku | Done; local MSSDK signing connects directly for chat, gifts, likes, entries, and common social events |
...
| Settings | Done; shared Bilibili/Douyu/Huya send switch, theme, proxy, QR/manual Cookie login, Huya manual Cookie input, custom IPTV M3U, quality preference, project link, and profile import/export |
...
Bilibili, Douyin, and Douyu support QR/manual Cookie storage; Huya supports manual Cookie storage. Bilibili, Douyu, and Huya use the shared `danmaku_send_enabled` switch, Cookie, text, and cooldown validation for single sends and room-session auto-send. Douyu reports local submission, `chatres(res=0)`, and the real room echo as separate states.

Payments, gifts, batch sending, auto-replies, automatic retry of an unknown result, and recording are not implemented.

## License

See the repository `LICENSE` when present.
