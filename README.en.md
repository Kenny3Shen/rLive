...
| Bilibili browse / categories / search / playback / danmaku | Done; a shared local send switch plus a valid Cookie enable one user-initiated normal chat message |
| Huya browse / rooms / playback / danmaku | Done; a shared local send switch plus a manually saved local Cookie enable one ordinary message per action |
| Douyu browse / rooms / playback / danmaku | Done; a shared local send switch plus a QR/manual local Cookie enable one ordinary message per action |
...
| Settings | Done; shared Bilibili/Douyu/Huya send switch, theme, proxy, QR/manual Cookie login, Huya manual Cookie input, custom IPTV M3U, quality preference, and profile import/export |
...
3. For **Bilibili danmaku**, open **Settings → Account** and use QR login or paste a Cookie manually. Normal chat preserves validated Bilibili CDN image emotes in order. After an unexpected disconnect, rLive rotates gateways, refreshes the short-lived token, and reconnects. To send on Bilibili, Douyu, or Huya, first enable **启用发送功能** at the top of **Settings → Account**. This default-off, device-local switch controls all three platforms but does not replace their own account requirements: Bilibili needs `SESSDATA` and `bili_jct`; Douyu needs a QR/manual Cookie with `acf_username`, `acf_stk`, and `acf_ltkid`; Huya currently needs a manually saved Cookie with a numeric account ID (`yyuid` or `udb_uid`) and an opaque login proof (`udb_n` or `udb_cred`). The composer appears in the middle of the player controls and submits on Enter or click. These are user-operated local-Cookie features, not claims of a public platform write API; verify acceptance in a permitted room and follow platform terms.
4. The room-side tabs are **Danmaku / SC / Follows / Settings**. In **Settings**, adjust display area, line count, opacity, font size and weight, speed, same-content grouping, gift filtering, and shield words. Changes apply immediately and shield words are saved automatically. Click a normal message to choose the icon-only **Copy** or **+1** actions: Copy writes to the clipboard; on Bilibili, Douyu, or Huya with sending prerequisites ready, **+1** submits the exact same text once (it never appends a literal `+1`). After a local send command succeeds, the list shows a distinct local-colour **you / awaiting platform echo** marker and floating danmaku is amber with `【我·待平台回显】`; neither is a platform confirmation, and a real echo keeps its original platform appearance.
...
rLive focuses on public lists, playback metadata, and receiving danmaku. Bilibili, Douyin, and Douyu QR/manual Cookie login plus Huya manual Cookie input are user initiated and stored locally. The default-off device-local **启用发送功能** switch jointly controls one ordinary user-initiated text message per action on Bilibili, Douyu, and Huya; it is not exported or imported with profiles. A local pending-platform-echo marker is not delivery or platform authorisation. Douyu/Huya local-Cookie sends do not establish a public platform API grant or guarantee delivery; users must verify the real service result in a permitted room and comply with current platform terms.
