...
> [!WARNING]
> 1.0 使用全新的版本化数据库，不会自动打开或迁移 0.x 的 `rlive.db`。升级前必须先用最后一个 0.x 过渡版本导出配置并备份录制；范围和步骤见 [1.0 兼容性清理](docs/zh/1.0兼容性清理.md)。
...
| 进程内 FFmpeg 录制 | 桌面端最多同时录制 4 路；直播流只重新封装、不转码，支持后台录制、按时长自动分割、磁盘保护、崩溃恢复和本地录制库 |
| 实时弹幕           | 同时提供列表、画面弹幕和 B 站 SC，支持屏蔽、礼物过滤和重复合并；发送能力需要本机登录与显式授权        |
| 多画面与 IPTV      | 桌面端支持 4 / 6 路布局、拖拽换位和独立音量；IPTV 支持公开频道、自有 M3U 和有权使用的 HTTP(S) 直链    |
...
| ----------------- | ---------------- | ---------------- | ------------------------- | ------------------------------- | ------------------------------------------- |
| Windows x64       | 支持             | 支持             | CPU / CUDA                | 支持                            | 本地 release 构建已验证；正式分发待合规审计 |
| Linux x86_64      | 支持             | 支持             | CPU / CUDA 路径待实机验证 | 代码支持，需匹配 FFmpeg runtime | 发行包仍需固定并打包 FFmpeg runtime         |
| macOS arm64 / x64 | 代码支持，未验证 | 代码支持，未验证 | CPU，未验证               | 代码支持，未验证                | 尚未完成真机、FFmpeg 打包、签名与公证验证   |
| Android arm64-v8a | 支持             | 不支持           | 不支持                    | 不支持                          | API 24 及以上；保留单直播间与 IPTV 体验     |
...
4. 桌面端可从直播间标题栏开始录制，也可在关注页右键直播中的主播直接开启后台录制；侧栏提供「多画面」「录制」和「IPTV」。
5. 在「设置 → 数据管理」查看或修改应用数据位置；位置切换在下次启动生效，不会自动迁移旧数据。
