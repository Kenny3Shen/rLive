...
3. **哔哩哔哩弹幕**：设置 → 账号 → 粘贴 Cookie → 进入房间后右侧列表与飘屏生效。若需发送，显式开启「实验性：发送 B 站弹幕」，缺少 `SESSDATA` / `bili_jct` 时发送框保持禁用；每条均需二次确认。
