...
| 不装客户端也能看 | 浏览器访问 | 让运行中的 rLive 把同一套界面提供给本机浏览器，浏览、播放与弹幕都可用 |
...
| Twitch | 桌面端尝试直播、分类、搜索分页；移动端首屏 | HLS；检测到服务端插播广告后尝试备用播放列表 | 匿名 IRC 接收，不支持发送 |
...
- Twitch 检测到服务端插播广告时，会在本机依次尝试备用播放列表并优先保留当前清晰度；实测 Twitch 按播放器类型区别插播，优先使用能保留完整清晰度的干净档位，必要时才降到受 `360p` 限制的档位。全部档位都被插播时画面可能短暂停顿，上游策略也可能随时变化。
...
- 可选的浏览器访问由本机 rLive 直接提供界面与命令，默认只监听 `127.0.0.1`，局域网访问需显式开启并使用一次性令牌。
- 桌面与 Android 统一使用完整动态效果，共用同一套页面转场、Overlay 和按压反馈。
...
Windows 安装包与便携 ZIP 都不内置约 `212 MiB` 的 ASR 运行库。首次启用本地字幕时，应用会连同所选模型下载运行库并校验完整性。安装版和便携版都把 `rlive.db`、`logs/`、`models/` 与 `asr-runtime/` 保存在 `%APPDATA%\rlive`，升级、卸载或移动 EXE 都不影响数据。
