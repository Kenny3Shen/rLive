...
| **哔哩哔哩** 分类 / 推荐 / 搜索 / 播放 / 弹幕 | 完成 Done；可灰度启用单条普通弹幕发送（需 Cookie + 二次确认） |
...
3. **哔哩哔哩弹幕**：设置 → 账号 → 粘贴 Cookie → 进入房间后右侧列表与飘屏生效。普通聊天支持按原顺序显示平台图片表情，只加载校验过的 B 站 CDN 图片；意外断线会轮换网关、刷新短时 token 后自动重连。若需发送，显式开启「实验性：发送 B 站弹幕」，缺少 `SESSDATA` / `bili_jct` 时发送框保持禁用；每条均需二次确认。
...
以列表、播放地址与接收弹幕为主；可选用户粘贴 Cookie。B 站实验性功能仅允许用户逐条确认的普通文本发送。
