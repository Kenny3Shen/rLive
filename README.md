...
| **斗鱼** 列表 / 房间 / 播放 / 弹幕                                                                                      | 已完成；个人直播间已成功发送，仍需完整本机 Cookie 并按平台确认/真实回显显示          |
| **抖音** 分类 / 推荐 / 房间 / 播放                                                                                     | 已完成；推荐 / 分类仅可靠支持 SSR 首屏；房间优先经 SSR 与公开 reflow 解析，搜索需完整登录 Cookie（可扫码或手动保存，仍可能受网页验证限制） |
| **抖音** 实时弹幕                                                                                                       | 已完成；在设置中配置签名服务后支持聊天 / 礼物 / 点赞 / 进场等常用事件                |
...
| 设置：侧栏亮 / 暗模式切换、代理、Cookie（B 站 / 抖音 / 斗鱼扫码或手动输入；虎牙手动输入）、抖音签名服务、IPTV M3U、清晰度偏好、关于（免责声明 / GitHub）、配置导入导出 | 已完成                                                                                |
...
| [docs/zh/抖音平台API文档.md](docs/zh/抖音平台API文档.md)                                 | 抖音平台接入、签名服务与边界  |
...
4. **弹幕设置与操作**：右侧标签依次为「弹幕 / SC / 关注 / 设置」；在「设置」中调整显示区域、行数、不透明度、字号、字重、速度、相同内容合并、礼物过滤和屏蔽词。滑块与屏蔽词输入都会即时生效，屏蔽词会自动保存。点击一条普通弹幕可选择图标「复制」或「+1」；复制会写入剪贴板，「+1」会提交完全相同的一条文本，不会附加“+1”。本机提交不等于送达，右侧列表与飘屏仅在收到平台真实回显后显示。
...
8. **抖音 / 快手 / Twitch**：抖音可匿名浏览首屏分类 / 推荐并播放；搜索需要在「设置 → 账号」选择扫码登录或保存完整网页 Cookie。推荐和分类当前只提供首屏，不会用 Cookie 或 SSR 偏移伪造「加载更多」；抖音网页访问验证仍可能限制扫码或搜索。房间优先从 SSR 取得内部房间号，再使用公开 reflow 数据解析播放信息。抖音弹幕需要在「设置 → 账号 → 抖音实时弹幕」填写你自行运行或信任的签名服务完整地址：允许 HTTPS，或 `localhost` / `127.0.0.1` / `::1` 的 HTTP 地址；请求不会跟随重定向，本机 HTTP 服务不使用全局代理。快手支持公开推荐、分类、游戏分区、房间与播放；搜索只匹配游戏分区名称，实时弹幕暂不支持。Twitch 支持直播列表、分类、搜索、房间、HLS 播放与匿名 IRC 弹幕；受公开接口限制，浏览结果仅可靠支持首屏，不提供翻页。
...
以列表、播放地址与接收弹幕为主；B 站、抖音、斗鱼支持用户主动扫码或手动保存本机 Cookie，虎牙支持手动输入本机 Cookie。B 站、斗鱼和虎牙的本机发送入口都只支持用户每次主动操作一条普通文本；斗鱼当前网页认证/确认流程已在个人直播间验证成功。运行时仍须将本地提交、平台 `chatres`（如适用）与真实回显区分开来。所有本机 Cookie 路径都不代表已获得公开平台 API 授权或一定送达，请在有权发言的房间使用，并遵守现行平台条款。
