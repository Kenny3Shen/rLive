...
| 短视频竖屏流 | 抖音公开作品链接/分享文字播放与 Cookie 推荐流（默认关闭的实验灰度）；B 站 story 流的沉浸式竖屏消费：上下滑动换片、点按暂停、自动循环、可拖动进度条（拖动时带时间与缩略图预览）、弹幕收看与发送、评论与详情抽屉 |
...
1. 顶部选择直播平台，从推荐、分类或搜索进入房间；B 站视频从「视频」页浏览或搜索，竖屏短视频从侧栏「短视频」进入。
...
| [代码审计与优化方向](docs/zh/代码审计与优化方向.md) | 性能与功能优化优先级、代码证据、验收方法与实施边界 |
