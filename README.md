...
8. **抖音 / 快手 / Twitch**：抖音可匿名浏览首屏分类 / 推荐并播放；搜索需要在「设置 → 账号」选择扫码登录或保存完整网页 Cookie。推荐和分类当前只提供首屏，不会用 Cookie 或 SSR 偏移伪造「加载更多」；抖音网页访问验证仍可能限制扫码或搜索。房间优先从 SSR 取得内部房间号，再使用公开 reflow 数据解析播放信息。抖音弹幕固定调用本机 `http://127.0.0.1:18080/sign`，请在进房前启动兼容的本机签名服务；请求不会使用全局代理或跳转到远程地址。快手支持公开推荐、分类、游戏分区、房间与播放；搜索只匹配游戏分区名称，实时弹幕暂不支持。Twitch 支持直播列表、分类、搜索、房间、HLS 播放与匿名 IRC 弹幕；受公开接口限制，浏览结果仅可靠支持首屏，不提供翻页。
