...
**rLive** 是一款桌面端直播聚合客户端，基于 **Tauri 2 + React + TypeScript + Rust**。灵感来自 [Simple Live](https://github.com/June6699/dart_simple_live) / [xiaoyaocz/dart_simple_live](https://github.com/xiaoyaocz/dart_simple_live)，为独立重写，**非**官方客户端。
...
| **虎牙** 列表 / 房间 / 播放 / 弹幕                                                                                      | 已完成；手动保存本机 Cookie 后支持手动单条与房间内会话级自动发送                    |
...
| 房间内弹幕设置（区域 / 行数 / 透明度 / 字号 / 字重 / 速度 / 重复 / 礼物过滤 / 屏蔽词 / 自动发送）                         | 已完成；自动发送仅在 B 站 / 斗鱼 / 虎牙房间会话中生效                              |
...
| 设置：侧栏亮 / 暗模式切换、代理、Cookie（B 站 / 抖音 / 斗鱼扫码或手动输入；虎牙手动输入）、IPTV M3U、清晰度偏好、关于（免责声明 / GitHub）、配置导入导出 | 已完成                                                                                |
...
3. **哔哩哔哩弹幕**：设置 → 账号 → 选择「扫码登录」或「手动输入」保存 Cookie → 进入房间后右侧列表与飘屏生效。普通聊天会按原顺序显示平台图片表情，只加载校验过的 B 站 CDN 图片；意外断线会轮换网关、刷新短时 token 后自动重连。若需发送，先在「设置 → 账号」开启默认关闭的「启用发送功能」，并保存含 `SESSDATA` / `bili_jct` 的 Cookie；发送框位于播放器功能栏中间。**斗鱼**可扫码或手动保存完整本机 Cookie；当前网页对齐的发送链路已在个人直播间成功验证。每次发送仍以 `chatres(res=0)` 作为平台接收信号，并以正常收弹幕连接的真实回显决定 UI 展示，详见[斗鱼平台 API 文档](docs/zh/斗鱼平台API文档.md)。**虎牙**当前需手动保存含数字账号 ID（`yyuid` 或 `udb_uid`）及登录凭据（`udb_n` 或 `udb_cred`）的本机 Cookie。所有发送入口均为用户主动操作的本机 Cookie 功能，不代表平台已提供公开写入 API；请只在有权发言的房间验证，并遵守平台条款。
4. **弹幕设置与操作**：右侧标签依次为「弹幕 / SC / 关注 / 设置」；在「设置」中调整显示区域、行数、不透明度、字号、字重、速度、相同内容合并、礼物过滤和屏蔽词。B 站、斗鱼、虎牙还可在此会话级开启「自动发送弹幕」：默认关闭且不保存，只有本机发送授权、对应 Cookie/可发送状态及文本校验均有效时可开启。开启后先等待 10 秒；文本会将换行和连续空白压缩为一个空格，按最多 15 个用户可见字符且不超过平台 UTF-16 上限拆段并循环发送，每次发送起始至少相隔 10 秒。编辑文本、切换房间（会清空该会话文本）、离开页面或任何发送失败都会立即停用并显示原因，失败不会自动重试。滑块与屏蔽词输入都会即时生效，屏蔽词会自动保存。点击一条普通弹幕可选择图标「复制」或「+1」；复制会写入剪贴板，「+1」会提交完全相同的一条文本，不会附加“+1”。本机提交不等于送达，右侧列表与飘屏仅在收到平台真实回显后显示。
...
8. **抖音 / 快手 / Twitch**：抖音可匿名浏览首屏分类 / 推荐并播放；搜索需要在「设置 → 账号」选择扫码登录或保存完整网页 Cookie。推荐和分类当前只提供首屏，不会用 Cookie 或 SSR 偏移伪造「加载更多」；抖音网页访问验证仍可能限制扫码或搜索。房间优先从 SSR 取得内部房间号，再使用公开 reflow 数据解析播放信息。抖音弹幕在本机完成 MSSDK 签名后直连官方 WSS，无需外部签名服务。快手支持公开推荐、分类、游戏分区、房间与播放；搜索只匹配游戏分区名称，实时弹幕暂不支持。Twitch 支持直播列表、分类、搜索、房间、HLS 播放与匿名 IRC 弹幕；受公开接口限制，浏览结果仅可靠支持首屏，不提供翻页。
...
以列表、播放地址与接收弹幕为主；B 站、抖音、斗鱼支持用户主动扫码或手动保存本机 Cookie，虎牙支持手动输入本机 Cookie。B 站、斗鱼和虎牙支持手动单条发送，以及由用户在房间右侧设置开启、默认关闭且不持久化的会话级自动发送；它仍复用同一授权、Cookie、文本和冷却校验。斗鱼当前网页认证/确认流程已在个人直播间验证成功。运行时仍须将本地提交、平台 `chatres`（如适用）与真实回显区分开来。所有本机 Cookie 路径都不代表已获得公开平台 API 授权或一定送达，请在有权发言的房间使用，并遵守现行平台条款。

**不做**支付、送礼、批量发送、自动回复、对未知结果的自动重试或录制。
