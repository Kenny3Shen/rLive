...
| 进程内 FFmpeg 录制 | 桌面端最多同时录制 4 路；直播流只重新封装、不转码，支持后台录制、独立弹幕轨、自定义目录和本地录制库   |
...
- **录制路径**：平台 CDN → 进程内 `ffmpeg-next` / libavformat → 本地录制目录。FFmpeg 只负责桌面录制，不参与前端播放，也不会启动 `ffmpeg.exe` CLI。
