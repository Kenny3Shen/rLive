...
fn download_prebuilt_libs(
    ...
    let lib_dir = extracted_library_dir(&extracted_dir, target_os, target_arch)?;
    ...
    let unpack_destination = if target_os == "android" {
    ...
    let unpack_result: Result<(), DynError> = (|| {
        ...
        archive.unpack(unpack_destination)?;
        Ok(())
    })();
    ...
    if !lib_dir.is_dir() {
        return Err(format!(
            "Downloaded archive did not contain the expected library directory: {}",
            lib_dir.display()
        )
        .into());
    }
...
fn android_abi(target_arch: &str) -> Result<&'static str, DynError> {
    match target_arch {
        ...
        "x86" => Ok("x86"),
        "x86_64" => Ok("x86_64"),
        _ => Err(format!("Unsupported Android target architecture: {target_arch}").into()),
    }
}

fn extracted_library_dir(
    extracted_dir: &Path,
    target_os: &str,
    target_arch: &str,
) -> Result<PathBuf, DynError> {
    ...
    Ok(extracted_dir.join("lib"))
}

fn stage_android_runtime_libs(lib_dir: &Path, target_arch: &str) -> Result<(), DynError> {
    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR")?);
    let tauri_dir = manifest_dir
        .parent()
        .and_then(Path::parent)
        .filter(|path| path.join("tauri.conf.json").is_file())
        .ok_or("Vendored sherpa-onnx-sys must live under src-tauri/vendor")?;
    let destination = tauri_dir
        .join("gen")
        .join("android")
        .join("app")
        .join("src")
    ...
    for name in ["libsherpa-onnx-c-api.so", "libonnxruntime.so"] {
        ...
        if !source.is_file() {
            return Err(format!(
                "Required sherpa-onnx Android library is missing: {}",
                source.display()
            )
            .into());
        }
    ...
    println!(
        "cargo:warning=Staged sherpa-onnx Android C API libraries from {}",
        lib_dir.display()
    );
    Ok(())
}
