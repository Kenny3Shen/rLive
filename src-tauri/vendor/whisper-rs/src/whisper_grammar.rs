...
// rLive downstream patch: when `WHISPER_DONT_GENERATE_BINDINGS=1`,
// whisper-rs-sys uses its bundled generated bindings on every target. Those
// bindings expose `whisper_gretype` as u32, including on Windows MSVC. Keeping
// this enum u32 preserves the C ABI and lets Windows builds avoid a host
// libclang dependency. The values are the C enum's stable 0..=6 range.
#[repr(u32)]
