...
/// Decode one WS binary frame into zero or more chat events.
pub fn decode_message(data: &[u8]) -> Vec<DanmakuEvent> {
    ...
    let push_bytes = match stream.read_bytes(1, false) {
        Ok(b) if !b.is_empty() => b,
        _ => return events,
    };
    let mut push = TarsReader::new(&push_bytes);
    ...
    let msg = push.read_bytes(2, false).unwrap_or_default();
    if msg.is_empty() {
        return events;
    }
    ...
    if uri == 1400 {
        ...
        let mut notice = TarsReader::new(&msg);
