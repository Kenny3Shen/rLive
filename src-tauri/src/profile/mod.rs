...
///
/// The danmaku-send toggle is an explicit consent for a write action, a custom
/// M3U URL can identify a private playlist or carry an access token, and a
/// Douyin signer receives the effective browser session while opening chat.
/// An imported profile must not be able to choose any of them.
...
pub fn merge_into_db(
    ...
    // choose a service that receives this device's Douyin web session.
