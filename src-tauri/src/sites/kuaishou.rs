...
impl LiveSite for KuaishouSite {
    ...
    async fn get_recommend_rooms(&self, _page: u32) -> AppResult<RoomListPage> {
        Err(not_implemented("kuaishou", "get_recommend_rooms"))
    }
    ...
    async fn get_category_rooms(
        ...
        Err(not_implemented("kuaishou", "get_category_rooms"))
    }

    async fn search_rooms(&self, _keyword: &str, _page: u32) -> AppResult<RoomListPage> {
        Err(not_implemented("kuaishou", "search_rooms"))
    }
    ...
    async fn get_play_urls(
        ...
        Err(not_implemented("kuaishou", "get_play_urls"))
    }

    async fn get_live_status(&self, _room_id: &str) -> AppResult<bool> {
        Err(not_implemented("kuaishou", "get_live_status"))
    }
}
